...
Make managed JavaLens servers discoverable across clients by generating MCP client configuration artifacts automatically (Cursor, Claude, Antigravity), so users do not have to manually register servers per tool.
